...
def clean_categories_data(df):
    ...
    for column in categories:
        ...
        categories[column] = categories[column].astype(int)  # Change np.int to int
...
def main():
    ...
    if len(sys.argv) == 4:
        messages_filepath, categories_filepath, database_filepath = map(Path, sys.argv[1:]) # Extract the parameters in relevant variable
